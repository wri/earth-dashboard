...
function DynamicTextWidget({ widget, showSource }) {
  ...
  useEffect(() => {
    if (parameters?.length > 0) {
      ...
      const paramsFound = text.match(paramRegexp).map(e => e.replace(/\{\{/g, "").replace(/\}\}/g, ""));
  ...
  const textElements =
    textData &&
    textData.splitArray.map(currentStr => {
      if (textData.paramsFound.includes(currentStr)) {
        ...
        return (
          <span className={styles.parameter} style={currentParam.style}>
            {textValue}
          </span>
        );
      } else {
  ...
  return (
    <div className={styles["c-dynamic-text-widget"]} {...(style && { style })}>
      <Spinner isLoading={loading} className="-relative -light" />
      <div className={styles["text-container"]}>{textElements}</div>
