...
function ShareBox({ url, topic, style, showInput, showBorder, onCopyLink }) {
  const getContent = (mobile = false, urlValue) => (
    ...
    >
      ...
      >
        <a
          href={`mailto:?subject=Earth Dashboard&body=I thought you'd be interested in what I found on the Earth Dashboard: ${urlValue}`}
        >
          <div
            className={classnames({
        ...
        <a
          href={`http://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(urlValue)}`}
          target="_blank"
        ...
        <a
          href={`https://twitter.com/share?url=${encodeURIComponent(
            urlValue
          )}&text=Check out what I found on the Earth Dashboard:`}
          target="_blank"
          rel="noopener noreferrer"
        >
          <div
  ...
  return (
    <div className={styles["c-share-box"]}>
      <MediaContextProvider>
        <Desktop>{getContent(false, url)}</Desktop>
        <Mobile>{getContent(true, url)}</Mobile>
      </MediaContextProvider>
    </div>
  );
}
