...
class LayerPopup extends PureComponent {
  ...
  componentWillMount() {
    ...
    if (layer) {
      ...
      if (!isEmpty(latlng) && !!layers.length && !!interactionConfig.config && !!interactionConfig.config.url) {
        ...
        <div className={styles["popup-content"]}>
          {(interaction.data || interactionState.data) &&
            output.map(outputItem => {
              ...
              const value = columnArray.reduce((acc, c) => acc[c], interaction.data || interactionState.data);
              return (
                <div className={styles["popup-field"]} key={outputItem.property || outputItem.column}>
                  <div className={styles["field-title"]}>{outputItem.property || outputItem.column}</div>
                  <div className={styles["field-value"]}>
                    ...
                    {(outputItem.property || outputItem.column) !== "Link" && this.formatValue(outputItem, value)}
                  </div>
                </div>
              );
            })}
