...
export const getCurrentLayerInteractions = createAsyncThunk(
  "LAYER-INTERACTIONS__GET-CURRENT-LAYER-INTERACTIONS",
  props => dispatch => {
    const { layer } = props;
    const {
...
export const getAvailableLayerInteractions = createAsyncThunk(
  ...
  props => dispatch => {
    ...
    if (layer && layer.provider !== "wms") {
      ...
      return fetchFields(url).then(response => {
