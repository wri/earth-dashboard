...
// utils
import { getColorByTopic, getSecondaryColorByTopic, getHeadlineSectionDataBytopic } from "utils/topics";
...
function HeadlineSection(props) {
  ...
  const getMainContainer = mobile => (
    <>
      <div className={styles["topic-name-title"]}>{topic?.toUpperCase()}</div>
      {mobile && <h2>{sectionData?.mainTitle}</h2>}
      {!mobile && <h1>{sectionData?.mainTitle}</h1>}
      <div className={styles.subtitle}>{sectionData?.subtitle}</div>
      <div className={styles["scroll-to-explore"]} style={{ color: topicSecondaryColor }}>
        <span
  ...
  return (
    <div className={styles["c-headline-section"]} style={{ backgroundColor: topicColor }}>
