...
// utils
import { getColorByTopic, getChallengeSectionDataByTopic } from "utils/topics";
...
function ChallengeToOurGlobalCommons(props) {
  ...
  const getPhotoAttribution = () => (
    <div className={styles["photo-attribution-container"]}>{data?.photoAttribution}</div>
  );
  ...
  return (
    ...
    >
      <MediaContextProvider>
        ...
        >
          <h1>
            The <span style={{ color: topicColor }}>Challenge</span> to our Global Commons
          </h1>
          {getParagraphs()}
          {getPhotoAttribution()}
        </Desktop>
