...
// utils
import { Mobile, Desktop, MediaContextProvider, DesktopLarge, DesktopXLarge, DesktopXXLarge } from "utils/responsive";

// constants
import { FRESHWATER_STEPS, FRESHWATER_STEPS_WORLDMAP } from "./constants";
...
function FreshwaterScrollyTelling() {
  ...
  const getWorldMapScrollama = () => (
    <Scrollama onStepEnter={onStepEnterWorldMap} offset={0.8}>
  ...
  const getVisualSources = (mobile, worldmapStory) => {
    const currentStep = worldmapStory
      ? FRESHWATER_STEPS_WORLDMAP[currentStepIndexWorldMap]
    ...
    return (
      ...
      >
        {currentStep.visualSource && <div className={styles.source}>Source: {currentStep.visualSource}</div>}
        {currentStep.visualDataset && <div className={styles.dataset}>Dataset: {currentStep.visualDataset}</div>}
      </div>
    );
  };
