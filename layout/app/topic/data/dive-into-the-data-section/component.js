...
// utils
import { getColorByTopic, getDiveIntoTheDataDataByTopic } from "utils/topics";
import { Mobile, Desktop, MediaContextProvider } from "utils/responsive";

// styles
import styles from "./dive-into-the-data-section.module.scss";

function DiveIntoTheDataSection({ topic, topicData }) {
  ...
  const getMainContainerContent = mobile => (
    <>
      {!mobile && (
        <h2>
          Dive into the <span style={{ color: topicColor }}>Data</span>
        </h2>
      )}
      {mobile && (
        <h3>
        ...
        {isBrowser && (
          <div className="row">
            {dataArray?.map(widget => (
              ...
              >
                <WidgetPanel widget={{ id: widget.id }} topic={topic} widgetShouldBeLoaded={true} />
              </div>
            ))}
          </div>
        )}
  ...
  return (
    <div id="dive-into-the-data" className={styles["c-dive-into-the-data-section"]}>
