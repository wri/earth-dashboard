...
function TopicNews(props) {
  ...
  const { loading, error, data, refetch } = useQuery(GET_NEWS_BY_TOPIC_QUERY(topic, limit), { variables: { topic } });
  ...
  return (
    <div className={styles["c-topic-news-mongabay"]}>
      ...
      {!loading &&
        !error &&
        data.posts.nodes.map(newsElem => (
          <div className={styles["news-item"]} key={`news-item-${newsElem.title}`}>
